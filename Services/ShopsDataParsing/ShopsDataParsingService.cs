    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            foreach (var shopDataRetriever in _shopDataRetrievers)
            {
                ...
                {
                    Console.WriteLine($"{product.Name}: {product.Price}UAH per {product.MeasureId} from {product.ShopId}, product {product.FullLinkProduct}, image {product.FullLinkImage}");
                }
            }
