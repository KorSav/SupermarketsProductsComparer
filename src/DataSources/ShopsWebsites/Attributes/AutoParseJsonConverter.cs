...
public class AutoParseJsonConverter<T> : JsonConverter<T> where T : class, new()
{
    public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        foreach (var property in properties)
        {
            ...
            {
                var parsedValue = JsonSerializer.Deserialize(value.GetRawText(), property.PropertyType, options);
                property.SetValue(instance, parsedValue);
            }
            else
            {
                throw new JsonException($"Missing required property '{jsonPropertyName}' for class '{typeof(T).Name}' in retrieved json.");
            }
        }
