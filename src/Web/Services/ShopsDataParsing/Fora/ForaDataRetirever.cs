...
namespace PriceComparer.Web.Services.ShopsDataParsing.Fora;
...
public class ForaDataRetriever : IShopDataRetriever
{
    ...
    public ForaDataRetriever(IConfiguration configuration, IHttpClientFactory httpClientFactory)
    {
        ...
        _baseUrl =
            configuration.GetSection($"ShopDataRetrievers:{Shop.Fora}:WebsiteUrl").Get<string>()
            ?? throw new InvalidOperationException(
                $"Url for {Shop.Fora} shop not found appsetting.json"
            );
