...
using PriceComparer.Web.Services.ShopsDataParsing.Exceptions;

namespace PriceComparer.Web.Services.ShopsDataParsing.Fozzy;
...
public partial class FozzyDataRetirever : IShopDataRetriever, IDisposable
{
    ...
    public FozzyDataRetirever(IConfiguration configuration)
    {
        ...
        _baseUrl =
            configuration.GetSection($"ShopDataRetrievers:{Shop.Fozzy}:WebsiteUrl").Get<string>()
            ?? throw new MissingOptionException("WebsiteUrl", Shop.Fozzy);
        _obligatoryParams = configuration
            .GetSection($"ShopDataRetrievers:{Shop.Fozzy}:ObligatoryQueryParams")
            .Get<Dictionary<string, string>>();
        _productsCountToRetrieve = configuration.GetSection("CountOfProductsToRetrieve").Get<int>();
