...
{
    public readonly partial struct ValidName(string name) : IEquatable<ValidName>
    {
        ...
        static string Validate(string name)
        {
            ...
            if (!LatinOrCyrrilicRegex().IsMatch(name))
                throw new ArgumentException(
                    $"User name should match regex: {LatinOrCyrrilicRegex()}"
                );
            ...
            get =>
                _value is not null
                    ...
                    : throw new InvalidOperationException(
                        $"Cannot instantiate struct {typeof(ValidName).FullName} using default value"
                    );
        }

        public readonly bool Equals(ValidName other) =>
        ...
        public override readonly bool Equals([NotNullWhen(true)] object? obj) =>
            obj is ValidName vn && vn.Equals(this);

        public static bool operator ==(ValidName left, ValidName right) => left.Equals(right);

        public static bool operator !=(ValidName left, ValidName right) => !(left == right);
    }
}
